...
def add_record():
    if request.method =='POST':
        ...
        debi = request.form.get('debi')  # Fixed: was 'debit' but form has 'debi'
        ...
        if not all([productname, price_per_unit, debi, quantity, amount]):
            flash(_("Please provide at least product name, price per unit, debit, quantity, and amount"),'info')
            return redirect(url_for('sale.displayall'))
...
def edit_record(id):
    ...
    sale = db.session.query(Sales).filter_by(salesid=id).first()

    if not sale:
        flash(_('Sale record not found'),'danger')
        return redirect(url_for('sale.displayall'))
        ...
        for k, v in changes.items():
            ...
            if k == 'date':
                try:
                    ...
                    setattr(sale, k, parse_date)
                    changes_made = True
                except ValueError:
                    flash(_('Invalid date format!'),'danger')
            elif k in sale.__table__.columns.keys():
                setattr(sale, k, v)
                changes_made = True
